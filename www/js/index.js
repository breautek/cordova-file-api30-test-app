...
let runTest = async (context) => {
    let resultNode = context.parentNode.parentNode.children[1].children[0];
    let testMethod = resultNode.id;
    let result;
    try {
        result = await window[testMethod]();
    }
    catch (ex) {
        log(testMethod);
        log(ex);
    ...
    switch (resultNode.tagName) {
        case 'IMG':
        case 'AUDIO':
            resultNode.src = result;
            break;
        case 'SPAN':
